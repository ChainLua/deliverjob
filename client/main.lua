...
RegisterNetEvent('esx_deliverjob:searchforjob')
		...
		for k,v in pairs(Config.JobPoints) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				...
				if not inMission then
					DrawText3D(v.x, v.y, v.z, _U('3d_text'))	
					if IsControlJustReleased(0, 182) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0) then
						TakeDeliver()
					end
				else
					DrawText3D(v.x, v.y, v.z, _U('cancel_text'))	
					if IsControlJustReleased(0, 182) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0) then
						...
						inMission = false
						TriggerServerEvent('esx_deliverjob:remove', 'item', 1, Config.Item)
						TriggerEvent('notification', _U('been_cancel'), 2)
					end
				end
		end
	end
...
function TakeDeliver()
	...
	TriggerServerEvent('esx_deliverjob:add', 'item', 1, Config.Item)
	TriggerEvent('notification', _U('started'), 1)
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 20.0 then
			...
			if IsControlJustReleased(0, 38) and d < 5.0 then
				...
				TriggerServerEvent('InteractSound_SV:PlayOnSource', 'knockDoor', 0.9)
				TaskPlayAnim( PlayerPedId(), 'mp_safehouselost@', 'package_dropoff', 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
				while IsEntityPlayingAnim(PlayerPedId(), "mp_safehouselost@', 'package_dropoff", 3) do Citizen.Wait(0); end          
				Citizen.Wait(2500)
				FreezeEntityPosition(PlayerPedId(), false)
				ClearPedTasks(PlayerPedId())
				TriggerEvent('notification', 'You dropped off the package and recieved $' .. reward, 1)
				TriggerServerEvent('esx_deliverjob:add', 'money', reward)
				TriggerServerEvent('esx_deliverjob:remove', 'item', 1, Config.Item)
			end
		end
	end
...
function SpawnVehicle()
	if not DoesEntityExist(jobVeh) then
	ESX.Game.SpawnVehicle(Config.VehicleModel, Config.VehicleSpawnPoint.coords, Config.VehicleSpawnPoint.heading, function(vehicle)
		jobVeh = vehicle
	end)
end
end
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
	local pcoords = GetEntityCoords(GetPlayerPed(-1))
	local d = GetDistanceBetweenCoords(pcoords, Config.VehicleSpawnPoint.newVehicle[1], Config.VehicleSpawnPoint.newVehicle[2], Config.VehicleSpawnPoint.newVehicle[3], true)
	if DoesEntityExist(jobVeh) and d < 4.0 or inMission and d < 4.0 then
		DrawText3D(Config.VehicleSpawnPoint.newVehicle[1],Config.VehicleSpawnPoint.newVehicle[2],Config.VehicleSpawnPoint.newVehicle[3], '[E] - Take new society vehicle')
		if IsControlJustReleased(0, 38) and d < 2.0 then
